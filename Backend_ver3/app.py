...
# This key is for OpenWeatherMap (weather data)
OPENWEATHER_API_KEY = "6d708c6c2b815be9da1c818e9468c7fc"

# --- Placeholder for a real soil API key ---
# You would replace this with the key from your chosen soil data provider.
SOIL_API_KEY = "YOUR_SOIL_API_KEY_HERE"

# *** CORRECTED MODEL LOADING LOGIC ***
# Only define the MODEL_PATH once at the top of the file
MODEL_PATH = "crop_yield_model.pkl" # Revert to the original name for simplicity
crop_yield_model = None
# Attempt to load the model. If it doesn't exist, the app will run, but the /optimize_farm endpoint will fail.
...
def optimize_farm():
    ...
    try:
        ...
        # 2. Predict Yield (Assumes 7 features: area, temp, humidity, N, P, K, pH)
        features = np.array([[
            farm['area'],
            weather['temperature'],
            weather['humidity'],
            soil_data['nitrogen_ppm'],
            soil_data['phosphorus_ppm'], # NEW FEATURE
            soil_data['potassium_ppm'],  # NEW FEATURE
            soil_data['ph_level']        # NEW FEATURE
        ]])
